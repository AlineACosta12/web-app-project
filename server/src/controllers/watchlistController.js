...
const updateWatchlistStatus = async (req, res) => {
  ...
  if (!status || !['plan', 'watching', 'watched'].includes(status)) {
    return res.status(400).json({ message: 'status must be plan, watching, or watched' })
  }
...
module.exports = { getWatchlist, addToWatchlist, updateWatchlistStatus, removeFromWatchlist }
